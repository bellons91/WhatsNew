namespace WhatsNew.Tests
{
    ...
    [IncludeRandomSelection("Records data type")]
    public class About_Records
    {
        ...
        /* 
		 ***** BEWARE OF RECORDS!! *****
		 Source: Jon Skeet's "Unexpected inconsistency in records"
        ...
        [Test]
        [DotNet5]
        public void Records_clone_not_via_constructor()
        {
            var number2 = new Number(2);
            Assert.That(number2.Even, Is.True);

            var number3 = new Number(3);
            Assert.That(number3.Even, Is.False);

            var number17 = number2 with { Value = 17 };
            Assert.That(number17.Even, Is.True); // WHAT??!! The clone does not call the constructor!
        }
